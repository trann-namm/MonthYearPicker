...
class MonthView extends ListView {
    ...
    public MonthView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) {
            _monthSelectedCircleSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                    _monthSelectedCircleSize, displayMetrics);
        } else {
            _monthSelectedCircleSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                    _monthSelectedCircleSize, displayMetrics);
        }
    ...
    private String getMonthName(int month) {
        ...
        if (_monthNames != null && _monthNames.length > 0)
            return _monthNames[month];

        return DateHelper.getMonthName(month, _monthFormat, _locale);
    ...
    private void drawDays(Canvas canvas) {
        ...
        for (int month = 0; month < 12; month++) {
            ...
            canvas.drawText(getMonthName(month), x, y, paint);
            j++;
            if (j == _numDays) {
                j = 0;
                y += _rowHeight;
    ...
    void setLocalFormats(Locale locale, String monthFormat) {
        _locale = locale;
        _monthFormat = monthFormat;
    }

    void setMonthNamesArray(String[] array) {
        _monthNames = array;
    }

    void setMonthSelectedCircleSize(int size) {
        DisplayMetrics displayMetrics = getContext().getResources().getDisplayMetrics();
        _monthSelectedCircleSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                size, displayMetrics);
    }

    void setMonthTextSize(int size) {
